...
function App() {
	...
	useEffect(() => {
		//set state default values onMount
		...
		if (!invalidRegex) {
			filteredCardNames = allCardNames.filter((cardName) => {
				...
				if (factionEquals) {
					...
					factionTrue = filteredFactions.some((faction) =>
	...
	return (
		<div className="App">
			<SearchArea
				...
				isRegex={isRegex}
				factionEquals={factionEquals}
				setFactionEquals={setFactionEquals}
				totalDisplayed={displayCardNames.length}
				onSearchChange={onSearchChange}
			/>
			{!invalidRegex ? (
				<DisplayArea
					displayCardNames={displayCardNames}
